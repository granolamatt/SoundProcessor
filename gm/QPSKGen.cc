...
pybind11::array_t<std::complex<float>> QPSKGen::getSymbols(pybind11::array_t<std::complex<float>> dataIn) {
    ...
    std::complex<float> *dIn = (std::complex<float> *) bufIn.ptr;
    //std::complex<float> *dataOut = (std::complex<float> *) bufOut.ptr;
    std::complex<float> *dataOut = new std::complex<float>[N];

    py::capsule free_when_done(dataOut, [](void *f) {
		    std::complex<float> *dataOut = reinterpret_cast<std::complex<float> *>(f);
		    std::cerr << " Freed the memory" << std::endl;
		    delete[] dataOut;
	});
